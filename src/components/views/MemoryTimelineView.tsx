...
export function MemoryTimelineView({ pairId }: MemoryTimelineViewProps) {
    ...
    return (
        <div className="flex-1 flex flex-col bg-background dark:bg-zinc-950 h-full relative overflow-hidden">
            {/* Header Area */}
            <div className="px-6 pt-10 pb-8 bg-gradient-to-b from-white dark:from-zinc-950 via-white dark/90 to-transparent sticky top-0 z-20">
                <div className="flex items-center justify-between mb-6">
                    <button onClick={() => navigate(-1)} className="p-2 bg-white dark:bg-zinc-900 shadow-md border border-slate-100 dark:border-zinc-800 rounded-full text-slate-600 dark:text-zinc-400 hover:scale-105 active:scale-95 transition-all">
                        <ArrowLeft size={24} />
                    </button>
                    <div className="px-4 py-1.5 bg-pastel-peach/10 dark:bg-pastel-peach/5 border border-pastel-peach/20 rounded-full">
                        <span className="text-xs font-bold text-pastel-peach tracking-wider uppercase">
                            {t('timeline.memoriesCount', { count: entries.length })}
                        </span>
                    </div>
                </div>
                <h1 className="text-4xl font-black text-slate-800 dark:text-zinc-100 mb-2 tracking-tight">
                    {t('timeline.title')}
                </h1>
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-24 relative">
                ...
                <div className="absolute left-10 top-0 bottom-0 w-1 bg-gradient-to-b from-pastel-peach/50 via-pastel-blue/50 to-pastel-mint/50 dark:from-pastel-peach/20 dark:via-zinc-800 dark:to-zinc-800 rounded-full z-0 opacity-50" />
